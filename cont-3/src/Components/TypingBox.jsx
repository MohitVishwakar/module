    ...
    if (currentWordRef) {
      ...
      if (e.keyCode === 32) {
        ...
        if (allCurrentChars.length <= currentCharIndex) {
          ...
          allCurrentChars[currentCharIndex - 1].classList.remove(
            "current-right"
          );
        } else {
          setMissedChars(
            missedChars + (allCurrentChars.length - currentCharIndex)
          ); // gives how many chars missed
        ...
        wordsSpanRef[currentWordIndex + 1].current.childNodes[0].className =
          "current"; //cursor visible at every first letter of word
      ...
      <div className="type-box" onClick={focusInput}>
        {testEnd ? (
          <div>
            <Stats
              ...
              graphData={graphData}
            />
            <button onClick={() => setTestEnd(false)} type="button">
              Try Again
            </button>
          </div>
        ) : (
        ...
        {!testEnd && (
          <center className="btn-container">
            ...
            <button className="bottom-btn" onClick={() => handleWords(10)}>
              10
            </button>
            {/* <button className='bottom-btn' onClick={()=>handleWords(25)}>25</button> */}
            <button className="bottom-btn" onClick={() => handleWords(50)}>
              50
            </button>
            <button className="bottom-btn" onClick={() => handleWords(80)}>
              80
            </button>
            <button className="bottom-btn" onClick={() => handleWords(100)}>
              100
            </button>
            <span className="bottom-span">-</span>
